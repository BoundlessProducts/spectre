//! Embedded runtime monitor generated from `bench-k20`.
//!
...
/// Embedded runtime monitor for the `bench-k20` specification.
///
...
impl Monitor {
    ...
    fn check_invariants(&self, action: &str) -> Vec<Violation> {
        ...
        // invariant `inv1`
        if !((self.state.aliceBalance >= 0)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv1".to_string(),
                message: "inv1".to_string(),
            });
        }
        // invariant `inv2`
        if !((self.state.bobBalance >= 0)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv2".to_string(),
                message: "inv2".to_string(),
            });
        }
        // invariant `inv3`
        if !((self.state.aliceBalance <= 1000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv3".to_string(),
                message: "inv3".to_string(),
            });
        }
        // invariant `inv4`
        if !((self.state.bobBalance <= 1000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv4".to_string(),
                message: "inv4".to_string(),
            });
        }
        // invariant `inv5`
        if !(((self.state.aliceBalance + self.state.bobBalance) >= 0)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv5".to_string(),
                message: "inv5".to_string(),
            });
        }
        // invariant `inv6`
        if !((self.state.aliceBalance <= 2000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv6".to_string(),
                message: "inv6".to_string(),
            });
        }
        // invariant `inv7`
        if !((self.state.bobBalance <= 2000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv7".to_string(),
                message: "inv7".to_string(),
            });
        }
        // invariant `inv8`
        if !(((self.state.aliceBalance + self.state.bobBalance) <= 2000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv8".to_string(),
                message: "inv8".to_string(),
            });
        }
        // invariant `inv9`
        if !((self.state.aliceBalance >= -(1))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv9".to_string(),
                message: "inv9".to_string(),
            });
        }
        // invariant `inv10`
        if !((self.state.bobBalance >= -(1))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv10".to_string(),
                message: "inv10".to_string(),
            });
        }
        // invariant `inv11`
        if !((self.state.aliceBalance <= 3000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv11".to_string(),
                message: "inv11".to_string(),
            });
        }
        // invariant `inv12`
        if !((self.state.bobBalance <= 3000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv12".to_string(),
                message: "inv12".to_string(),
            });
        }
        // invariant `inv13`
        if !(((self.state.aliceBalance + self.state.bobBalance) <= 3000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv13".to_string(),
                message: "inv13".to_string(),
            });
        }
        // invariant `inv14`
        if !((self.state.aliceBalance >= -(2))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv14".to_string(),
                message: "inv14".to_string(),
            });
        }
        // invariant `inv15`
        if !((self.state.bobBalance >= -(2))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv15".to_string(),
                message: "inv15".to_string(),
            });
        }
        // invariant `inv16`
        if !((self.state.aliceBalance <= 4000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv16".to_string(),
                message: "inv16".to_string(),
            });
        }
        // invariant `inv17`
        if !((self.state.bobBalance <= 4000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv17".to_string(),
                message: "inv17".to_string(),
            });
        }
        // invariant `inv18`
        if !(((self.state.aliceBalance + self.state.bobBalance) <= 4000000)) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv18".to_string(),
                message: "inv18".to_string(),
            });
        }
        // invariant `inv19`
        if !((self.state.aliceBalance >= -(3))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv19".to_string(),
                message: "inv19".to_string(),
            });
        }
        // invariant `inv20`
        if !((self.state.bobBalance >= -(3))) {
            out.push(Violation {
                step: self.step_count,
                action: action.to_string(),
                property: "inv20".to_string(),
                message: "inv20".to_string(),
            });
        }
        out
    }
