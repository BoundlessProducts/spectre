...
### State Caching (`--use-cache`)

The `--use-cache` flag saves the fully-explored BFS state graph to disk after the first run and restores it on subsequent runs.  When the spec has not changed, cache restore skips the entire BFS traversal:

```bash
# First run: explore and cache
spectre verify examples/raft-election-safety.spec --max-states 30000 --use-cache

# Subsequent runs: restore from cache (33× faster for Raft / 11× for bank-account)
spectre verify examples/raft-election-safety.spec --max-states 30000 --use-cache
```

Cache files are stored adjacent to the spec file (e.g. `raft-election-safety.spec.cache`).  The cache is invalidated automatically if the spec file changes.

---

### Incremental Re-verification (`--incremental`)

When a single action changes (e.g. after `spectre sync` reports a `possibly-unsafe` mutation), re-verification of the entire state space is unnecessary.  `--incremental` re-verifies only the changed action in five steps:

1. **Prune** — remove all transitions produced by the changed action from the cached graph
2. **Reachability BFS** — recompute which states are still reachable from initial states
...
5. **BFS-expand** — expand newly reachable states with all actions, capped at the original bound
...
**Speedups on Apple M3 Pro (macOS 15.6, Go 1.24):**

| Spec | Changed action | Cold BFS | Incremental | Speedup |
|------|---------------|----------|-------------|---------|
| bank-account (3,745 states) | `freeze` | 2.1 s | 0.27 s | **7.7×** |
| Raft (22,432 states) | `stepDown1_sees2` | 8.7 s | 2.4 s | **3.6×** |
| Raft (22,432 states) | `vote2for1` | 8.7 s | 2.9 s | **3.0×** |

The speedup ratio depends on how many states are only reachable via the changed action — when that fraction is high, most of the graph can be reused.

---

### Semantic Sync (`spectre sync`)

`spectre sync` compares the current Rust source against an existing `.spec` file and classifies every change:

```bash
spectre sync examples/rust/bank_account.rs --spec examples/bank-account-parameterized.spec
```

Each field and action is classified as one of:

| Symbol | Meaning |
|--------|---------|
| `[=]` | Structurally unchanged — guards and assignments are semantically identical |
| `[+]` | New element — spec update required |
| `[-]` | Removed element — possibly unsafe |
| `[?]` | Semantic change detected — re-verification required |

`sync` compares: struct fields, action guard counts, guard expression strings, and normalised assignment-body expressions.  It detected **30/35 injected mutations** in evaluation (85.7%).

---

### Coverage-Guided Model-Based Testing (`--coverage-mode`)

The `--emit-traces` flag writes ITF execution traces for use with `spectre generate-driver`.  The `--coverage-mode` flag controls which paths are prioritised:
...
| Mode | Strategy |
|------|---------|
| `action` | Prefers actions not yet covered in the current trace (default) |
| `transition-pair` | Prefers (prev, next) action pairs not yet seen |
| `boundary` | Steers toward maximum values of integer variables |
| `rare-action` | Prefers globally least-executed actions |
| `property` | Steers toward states where integer variables are near invariant boundaries |
